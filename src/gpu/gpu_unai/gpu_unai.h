 ...
 *
 * When FastLightingEnabled() returns true, lighting calculations use
 * simple multiply+shift instead of LUT lookups.
 * Formula: (component * light) >> 4 instead of LightLUT[tex_idx | light_idx]
 *
 * Speedup: ~10-20% for lit textured polygons
 * Visual impact: Slight brightness differences (usually acceptable)
 * Controlled via menu: "Fast Light" option (OFF by default)
 */
