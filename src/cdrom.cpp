...
#define CDDA_BATCH_SIZE 32
	if (!Config.Cdda && !cdr.Muted) {
		...
		static unsigned char cdda_expect_m = 0, cdda_expect_s = 0, cdda_expect_f = 0;

		/* Detect seek: if position doesn't match expected, invalidate batch */
		if (cdr.SetSectorPlay[0] != cdda_expect_m ||
		    cdr.SetSectorPlay[1] != cdda_expect_s ||
		    cdr.SetSectorPlay[2] != cdda_expect_f) {
		...
		/* Feed current sector from batch */
		SPU_playCDDAchannel((short *)(cdda_batch + cdda_batch_pos * CD_FRAMESIZE_RAW),
		                    CD_FRAMESIZE_RAW);
		cdda_batch_pos++;

		/* Calculate expected next position (after SetSectorPlay increment below) */
		cdda_expect_f = cdr.SetSectorPlay[2] + 1;
		cdda_expect_s = cdr.SetSectorPlay[1];
		cdda_expect_m = cdr.SetSectorPlay[0];
		if (cdda_expect_f >= 75) { cdda_expect_f = 0; cdda_expect_s++; }
		if (cdda_expect_s >= 60) { cdda_expect_s = 0; cdda_expect_m++; }
	}
#undef CDDA_BATCH_SIZE
