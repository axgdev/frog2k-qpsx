...
// clk cycle byte
//
// v074 FIX: Scale sio_cycle by cycle_multiplier to maintain proper SIO timing
// at high CPU speeds. Without this fix, SIO interrupts fire too quickly
// relative to PSX kernel expectations when cycle_multiplier > 512 (~2x).
// The kernel waits ~100 cycles after sending a byte before acknowledging IRQ7.
// At 3x speed (768), unscaled 535 cycles = only ~178 instructions, too fast!
#ifdef PSXREC
...
#ifdef PSXREC
	// Scale SIO timing proportionally to cycle_multiplier
	// At cycle_multiplier=256 (1x): 535 cycles
	// At cycle_multiplier=768 (3x): 1605 cycles (same real-time delay)
	u32 adjusted_cycles = (psxSio.sio_cycle * cycle_multiplier) >> 8;
	psxEvqueueAdd(PSXINT_SIO, adjusted_cycles);
#else
...
{
	SIO_LOG("CreateMcd(%s) start", filename ? filename : "NULL");

	if (filename == NULL || filename[0] == '\0') {
		SIO_LOG("CreateMcd: NULL or empty filename!");
		printf("Error: NULL or empty filename parameter in %s\n", __func__);
		return -1;
	}

	if (!overwrite_file && FileExists(filename)) {
		SIO_LOG("CreateMcd: file exists, skipping");
#ifdef DEBUG_MEMCARDS
	...
	FILE *f = NULL;
	/* SF2000: Use static buffer - 128KB on stack causes crash on bare metal */
	static char mcd_data[MCD_SIZE];
	SIO_LOG("CreateMcd: calling InitMcdData");
	InitMcdData(mcd_data);
	SIO_LOG("CreateMcd: InitMcdData done, opening file");

	f = fopen(filename, "wb");
	if (f == NULL) {
		SIO_LOG("CreateMcd: fopen failed!");
		goto error;
	}
	SIO_LOG("CreateMcd: fopen OK, writing %d bytes", MCD_SIZE);

	if (fwrite(mcd_data, 1, MCD_SIZE, f) != MCD_SIZE) {
		SIO_LOG("CreateMcd: fwrite failed!");
		goto error;
	}
	SIO_LOG("CreateMcd: fwrite OK, flushing");

	if (fflush(f)) {
		SIO_LOG("CreateMcd: fflush failed!");
		goto error;
	}
	...
	SIO_LOG("CreateMcd: fsync OK");
#endif

	SIO_LOG("CreateMcd: closing file");
	if (fclose(f)) {
		SIO_LOG("CreateMcd: fclose failed!");
		f = NULL;
		goto error;
	}

	SIO_LOG("CreateMcd: SUCCESS");
	return 0;

error:
	SIO_LOG("CreateMcd: ERROR!");
...
{
	SIO_LOG("LoadMcd(%d, %s) start", mcd_num, filename ? filename : "NULL");
	...
	if (filename == NULL) {
		SIO_LOG("LoadMcd: NULL filename, slot empty");
	...
	if (*mc.filename == 0) {
		...
		SIO_LOG("LoadMcd: using default card name %s", mc.filename);
