...
{
	int m, s, d;
/* SF2000: MIPS requires byte-by-byte access to avoid unaligned exception */
#if defined(__arm__) || defined(__mips__) || defined(mips) || defined(SF2000)
...
int GetCdromFile(u8 *mdir, u8 *time, const char *filename) {
	...
	int retval = -1;
	/* SF2000: Use static buffer - 4KB on stack causes crash */
	static u8 ddir[4096];
	u8 *buf;
	int i;
...
int CheckCdrom() {
	...
	/* SF2000: Use static buffer - 4KB on stack causes crash */
	static unsigned char mdir[4096];
	char exename[256];
	int i, len, c;

	MISC_LOG("CheckCdrom() start");

	FreePPFCache();
	MISC_LOG("FreePPFCache done");

	time[0] = itob(0);
	time[1] = itob(2);
	time[2] = itob(0x10);

	MISC_LOG("Calling READTRACK...");
	READTRACK();
	MISC_LOG("READTRACK done, buf=%p", (void*)buf);
	...
	MISC_LOG("CdromLabel: %.32s", CdromLabel);
	...
	MISC_LOG("Calling READDIR...");
	READDIR(mdir);
	MISC_LOG("READDIR done");

	MISC_LOG("Looking for SYSTEM.CNF...");
	if (GetCdromFile(mdir, time, "SYSTEM.CNF;1") != -1) {
		READTRACK();
...
#ifdef NO_ZLIB
// Raw file implementation for platforms without zlib (like SF2000)
static void *raw_open(const char *name, boolean writing)
{
	if (!name || name[0] == '\0') {
		printf("Error: NULL ptr or empty filename passed to raw_open()\n");
		return NULL;
	}
	FILE *f = fopen(name, writing ? "wb" : "rb");
	if (!f) {
		printf("Error in raw_open() opening file %s\n", name);
	}
	return (void*)f;
}

static int raw_read(void *file, void *buf, u32 len)
{
	return (int)fread(buf, 1, len, (FILE*)file);
}

static int raw_write(void *file, const void *buf, u32 len)
{
	return (int)fwrite(buf, 1, len, (FILE*)file);
}

static long raw_seek(void *file, long offs, int whence)
{
	if (fseek((FILE*)file, offs, whence) != 0) return -1;
	return ftell((FILE*)file);
}

static int raw_close(void *file)
{
	return fclose((FILE*)file);
}

struct PcsxSaveFuncs SaveFuncs = {
	raw_open, raw_read, raw_write, raw_seek, raw_close
